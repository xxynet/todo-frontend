...
- **后端连接**：登录页可选填后端地址；留空时使用当前站点同源的 `/api/v1`，填写 `http(s)` 地址时连接指定后端
...
打开 <http://localhost:5173>。后端地址可留空，默认经同源 `/api/v1` 请求后端。

> 开发服务器内置代理：后端地址留空走 Vite 代理（目标 `http://127.0.0.1:5236`，可用环境变量 `VITE_PROXY_TARGET` 覆盖）；填写了具体地址则直连后端（需后端启用 CORS，默认 `*` 已允许）。
...
将 `dist/` 放入 todo-api 的 `data/dist` 后，前端与 API 由同一服务提供，可直接留空后端地址。独立部署 `dist/` 时，需将 `/api` 反向代理到后端，或在登录页填写后端地址。
